...
const FormComponent = () => {
  ...
  const {
    ...
    mutationFn: (newPost: CheftLoginOrSignUpType) =>
      fetch(
        formOperationState === "sign-up"
          ? "https://culinary-canvas-delta.vercel.app/api/chef"
          : "https://culinary-canvas-delta.vercel.app/api/chef/signup",
        {
          method: "POST",
    ...
    <div className="flex justify-center items-center h-6/6 w-5/5 lg:w-4/5 lg:h-5/6 font-serif">
      <div className="w-1/2 bg-pink-500 h-full hidden lg:flex flex-col justify-center gap-4 p-14 text-white rounded-l-2xl">
        ...
        <div className="text-xl font-semibold flex flex-col gap-4 text-black">
          ...
          <p className="flex gap-3 items-center">
            <Check className="text-gray-500" /> Exhibit your cooking skills in a
            very fun way
          </p>
          <p className="flex gap-3 items-center">
            <Check className="text-gray-500" /> Interact with like minded people
          </p>
        </div>
      </div>

      <div className="bg-black w-2/2 lg:w-1/2 h-full text-pink-500 flex items-center justify-center flex-col gap-2 p-14 rounded-r-2xl overflow-y-scroll">
